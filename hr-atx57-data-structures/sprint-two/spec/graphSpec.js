describe('graph', function() {
  ...
  it('should add and remove edge from both nodes', function() {
    graph.addNode(42);
    graph.addNode(9);
    graph.addEdge(42, 9);
    expect(graph.hasEdge(42, 9)).to.equal(true);
    expect(graph.hasEdge(9, 42)).to.equal(true);
    graph.removeEdge(42, 9);
    expect(graph.hasEdge(42, 9)).to.equal(false);
    expect(graph.hasEdge(9, 42)).to.equal(false);
  });
});
